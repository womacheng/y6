...
def evaluate_test_set(model, dataset, loss_fn, device):
    """封装测试集评估逻辑"""
    model.eval()
    dataset.set_split('test')
    batch_generator = generate_batches(dataset,
    ...
    with torch.no_grad():
        for batch_dict in batch_generator:
            ...
            # 计算损失
            loss = loss_fn(y_pred, batch_dict['y_target'])
            total_loss += loss.item() * len(batch_dict['y_target'])

            # 计算准确率
            _, predicted = torch.max(y_pred, 1)
            total_correct += (predicted == batch_dict['y_target']).sum().item()
            total_samples += len(batch_dict['y_target'])
...
results = {
    ...
    'best_epoch': train_state['epoch_index']
}
...
d = predict_nationality("McMahan", classifier, vectorizer)
print(d)

d = predict_nationality("Nakamoto", classifier, vectorizer)
print(d)

d = predict_nationality("Wan", classifier, vectorizer)
print(d)
